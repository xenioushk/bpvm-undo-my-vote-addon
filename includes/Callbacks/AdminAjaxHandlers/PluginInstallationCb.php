...
namespace UMVADDON\Callbacks\AdminAjaxHandlers;

use UMVADDON\Traits\PluginInstallationTraits;
...
class PluginInstallationCb {
	...
	public function save() {
		...
		$product_id = UMVADDON_PRODUCT_ID;
		$ip         = $_SERVER['REMOTE_ADDR'];
		$ver        = UMVADDON_PLUGIN_VERSION;
		...
		if ( is_array( $output ) && ! is_wp_error( $output ) && wp_remote_retrieve_response_code( $output ) === 200 ) {
			...
			if ( isset( $output_decode['status'] ) && $output_decode['status'] != 0 ) {
				...
				update_option( UMVADDON_PRODUCT_INSTALLATION_TAG, 1 ); // change the tag
